...
function Dashboard() {
  ...
  for (let i = 1; i <= 10; i++) {
    const filteredData = filteredValues(definicao).filter(
      (Processo) => Processo.assessor === i
    );
  ...
  const meta = filteredData.filter(
    (Processo) => Processo.meta === "sim"
  ).length;
  const prioridade = filteredData.filter(
    (Processo) => Number(Processo.prioridade) === 1
  ).length;
  const vinculado = filteredData.filter(
    (Processo) =>
      Processo.vinculado === "PRINCIPAL" || Processo.vinculado === "principal"
  ).length;
  ...
  const handleDetalheClick = (index: Array<procType>) => () => {
    ...
    if (index.length > 0) {
      ...
      index.forEach((item) => {
        const processNumberDiv = document.createElement(
          "div"
        ) as HTMLDivElement;
      ...
      resultContainer.innerHTML =
        "Usuário não tem processos atribuídos nesta definição";
    }
    console.log();
  };
  ...
  return (
    <>
      ...
      <div className="pie-chart-container">
        ...
        <div className="column" id="result-container"></div>
      </div>
    </>
  );
}
