...
const Sidebar = () => {
  ...
  return (
    <>
<div className={`bg-tertiary md:bg-primary block w-${isExpanded ? '60' : '18'} border border-secondary md:border-none text-white h-screen transition-all duration-500`}>
        <div className={`shadow h-[6rem] flex items-center justify-start transition-all duration-500 p-0.5`}>
          <img
          ...
          />
          {isExpanded && <h1 className="text-white font-bold font-title text-xl text-center ml-1">Controle e-Contas</h1>}
        </div>
        ...
        <Link
        to={item.to}
        className={`hover:scale-105 hover:shadow hover:text-slate-50 p-2 flex items-center justify-start text-gray-400 font-highlight text-l font-semibold h-15 hover:bg-secondary ${selectedItem === item.label ? 'shadow text-slate-50 bg-secondary bg-opacity-100' : ''}`}
